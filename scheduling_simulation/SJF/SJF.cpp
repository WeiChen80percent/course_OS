...
void random_process(vector<process> &process_queue){
    ...
    for(int i=0;i<10;i++){
        ...
        cout<<new_process.identification<<" "<<new_process.arrival_time<<" "<<new_process.burst_time<<endl; // Print process details
    }
}

// Function to sort processes based on arrival time and burst time at specific time intervals
void sort_process(vector<process> &process_queue,int current_time){
    if(current_time < 5){
        sort(process_queue.begin(),process_queue.end(),[](process a, process b){ 
            if(a.arrival_time < 5 && b.arrival_time < 5){
                return a.burst_time < b.burst_time; // Sort by burst time for processes arriving before time 5
            }
            else if(a.arrival_time < b.arrival_time){
                return true; // Sort by arrival time if one arrives earlier
            }
            else{
                return false;
            }
        ...
        sort(process_queue.begin(),process_queue.end(),[](process a, process b){
            if(a.arrival_time < 10 && b.arrival_time < 10){
                return a.burst_time < b.burst_time; // Sort by burst time for processes arriving before time 10
            }
            else if(a.arrival_time < b.arrival_time){
                return true; // Sort by arrival time if one arrives earlier
            }
        ...
        sort(process_queue.begin(),process_queue.end(),[](process a, process b){
            return a.burst_time < b.burst_time; // Sort all processes by burst time after time 10
        });
    }
}
...
void SJF(vector<process> process_queue){
    ...
    sort_process(process_queue,current_time); // Initial sorting of processes
    while(1){
        if(current_time == 5 || current_time == 10){
            sort_process(process_queue,current_time); // Re-sort at time 5 and 10
        }
        if(current_time < process_queue[0].arrival_time){
            gantte.push_back(0); // Add idle state to Gantt chart
            current_time++; // Increment time
            continue;
        }
        current_time++; // Increment time
        process_queue[0].burst_time--; // Decrease burst time of current process
        gantte.push_back(process_queue[0].identification); // Add process ID to Gantt chart
        if(process_queue[0].burst_time == 0){
            completion[process_queue[0].identification]=current_time; // Record completion time
            process_queue.erase(process_queue.begin()); // Remove completed process
        }
